...
trn_idxs <- sample(idxs, 7000) # CHANGE before submitting
...
sum(hatvalues(bryan_models$additive) > 2 * mean(hatvalues(bryan_models$additive)))

sum(hatvalues(bryan_models$selected_aic) > 2 * mean(hatvalues(bryan_models$selected_aic)))

sum(hatvalues(bryan_models$selected_bic) > 2 * mean(hatvalues(bryan_models$selected_bic)))

sum(hatvalues(bryan_models$log_additive) > 2 * mean(hatvalues(bryan_models$log_additive)))

sum(hatvalues(bryan_models$log_selected_aic) > 2 * mean(hatvalues(bryan_models$log_selected_aic)))

sum(hatvalues(bryan_models$log_selected_bic) > 2 * mean(hatvalues(bryan_models$log_selected_bic)))

sum(hatvalues(bryan_models$interactive) > 2 * mean(hatvalues(bryan_models$interactive)))

sum(hatvalues(bryan_models$log_interactive) > 2 * mean(hatvalues(bryan_models$log_interactive)))
...
```{r}
sum(abs(rstandard(bryan_models$additive)) > 2)

sum(abs(rstandard(bryan_models$selected_aic)) > 2)

sum(abs(rstandard(bryan_models$selected_bic)) > 2)

sum(abs(rstandard(bryan_models$log_additive)) > 2)

sum(abs(rstandard(bryan_models$log_selected_aic)) > 2)

sum(abs(rstandard(bryan_models$log_selected_bic)) > 2)

sum(abs(rstandard(bryan_models$interactive)) > 2)

sum(abs(rstandard(bryan_models$log_interactive)) > 2)
...
```{r}
sum(cooks.distance(bryan_models$additive) > 4 / length(resid(bryan_models$additive)))

sum(cooks.distance(bryan_models$selected_aic) > 4 / length(resid(bryan_models$selected_aic)))

sum(cooks.distance(bryan_models$selected_bic) > 4 / length(resid(bryan_models$selected_bic)))

sum(cooks.distance(bryan_models$log_additive) > 4 / length(resid(bryan_models$log_additive)))

sum(cooks.distance(bryan_models$log_selected_aic) > 4 / length(resid(bryan_models$log_selected_aic)))

sum(cooks.distance(bryan_models$log_selected_bic) > 4 / length(resid(bryan_models$log_selected_bic)))

sum(cooks.distance(bryan_models$interactive) > 4 / length(resid(bryan_models$interactive)))

sum(cooks.distance(bryan_models$log_interactive) > 4 / length(resid(bryan_models$log_interactive)))
...
```{r}
add_refit = lm((bryan_models$additive)$call$formula, 
               data = loans_train,
               subset = cooks.distance(bryan_models$additive) <= 4 / length(resid(bryan_models$additive)))

summary(bryan_models$additive)$adj.r.squared
summary(add_refit)$adj.r.squared

knitr::kable(data.frame("Predictor" = names(coef(add_refit)),
           "full data" = unname(coef(bryan_models$additive)),
...
## smaller data set.
models = generate_models(loans_data_small)

additive_diag = diagnostics(models$additive)
selected_aic_diag = diagnostics(models$selected_aic)
selected_bic_diag = diagnostics(models$selected_bic)

log_additive_diag = diagnostics(models$log_additive)
log_selected_aic_diag = diagnostics(models$log_selected_aic)
log_selected_bic_diag = diagnostics(models$log_selected_bic)

interactive_diag = diagnostics(models$interactive)
log_interactive_diag = diagnostics(models$log_interactive)
...
rmse_df = data.frame("Model" = c("Additive", "Add AIC", "Add BIC", "Log", "Log AIC", "Log BIC", "Interaction", "Log Interaction"),
                     "Train RMSE" = c(rmse(bryan_models$additive, loans_train),
                                      ...
                                      rmse(bryan_models$log_additive, loans_train, log = TRUE),
                                      rmse(bryan_models$log_selected_aic, loans_train, log = TRUE),
                                      rmse(bryan_models$log_selected_bic, loans_train, log = TRUE),
                                      rmse(bryan_models$interactive, loans_train),
                                      rmse(bryan_models$log_interactive, loans_train, log = TRUE)),
                     "Test RMSE" = c(rmse(bryan_models$additive, loans_test),
                                      ...
                                      rmse(bryan_models$log_additive, loans_test, log = TRUE),
                                      rmse(bryan_models$log_selected_aic, loans_test, log = TRUE),
                                      rmse(bryan_models$log_selected_bic, loans_test, log = TRUE),
                                      rmse(bryan_models$interactive, loans_test),
                                      rmse(bryan_models$log_interactive, loans_test, log = TRUE)))
...
Overall, the additive model family was slightly better at predicting the test dataset than the logarithmic, and both families slightly outperformed the interaction models.

However, these test RMSE values are indicators of poor prediction performance, as the standard deviation of the `interest_rate` in the test dataset of 4.968. Thus, our average error is more than one standard deviation away from the true value.
